...
`;
    ...
    const defaultLogger = new StringLogger();

    const 重紐韻 = [...'支脂祭真仙宵清侵鹽'];
    // 編寫反切規則參考了潘悟雲《反切行為與反切原則》
    const generate呼 = (母, 組, 韻, 上字呼, 下字呼, 下字組) => {
        let 呼;
    ...
    // 類需特殊處理，故寫法與上述兩函式不同
    const rawGenerate類 = (下字音韻地位, 母, 組, 韻, 上字類, 呼, 等) => {
    ...
    const 執行反切 = (上字音韻地位, 下字音韻地位) => {
        ...
        for (const 呼 of 所有呼) {
            for (const 等 of 所有等) {
                const 條件 = 所有呼.length > 1 || 所有等.length > 1
                    ? `當呼為${呼}口、等為${等}等時，`
                    : 所有呼.length > 1
                        ? `當呼為${呼}口時，`
                        : 所有等.length > 1
